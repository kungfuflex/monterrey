    ...
    async tick() {
        ...
        if (blockNumber < current) {
            ...
            for (const token in this.token_conversion_rate) {
                console.log("IN it");
                let oldBalances = await (0, exports.checkTokenBalances)(wallets, token, this.provider, newBlockNumber);
                let newBalances = await (0, exports.checkTokenBalances)(wallets, token, this.provider, blockNumber);
                for (const [diff, i] of newBalances.map((v, i) => [
                    v - oldBalances[i],
                    i,
                ])) {
