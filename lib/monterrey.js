...
const keygen = async (password, salt) => {
    return await new Promise((resolve, reject) => (0, pbkdf2_1.default)(password, salt, 1, 32, (err, result) => err ? reject(err) : resolve(ethers.toBeHex(result))));
};
...
class Monterrey extends events_1.EventEmitter {
    ...
    async getBalance(key) {
        return await this._backend.get((0, exports.toBalanceKey)(key));
    }
    async getWallets() {
