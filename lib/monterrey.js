    ...
    async tick() {
        ...
        if (blockNumber < current) {
            ...
            for (const token in this.token_conversion_rate) {
                let oldBalances = await (0, exports.checkTokenBalances)(wallets, this.provider, blockNumber, token);
                let newBalances = await (0, exports.checkTokenBalances)(wallets, this.provider, newBlockNumber, token);
                for (const [diff, i] of newBalances.map((v, i) => [
                    v - oldBalances[i],
                    i,
                ])) {
